...
% The distinction to be made, between interesting intervals on one hand and less striking parts of a video on the other is not likely to be a very strict one. After all, unedited video captures a single stretch of space and time, so any event that is of particular interest will unlikely have hard cut-off points in time. Rather, if we imagine an individual's interest as a function of time in a particular video, we would expect a somewhat continuous flowing line with some hills or spikes every now and then when an interesting event occurs. 
% 
...
\begin{figure}[htbp]
  \centering
    \includegraphics[width=\textwidth]{img/focus_selection}
